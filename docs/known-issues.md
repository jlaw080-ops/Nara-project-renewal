...
- **예정 보도를 준공으로 읽는다.** `[2023-01-12] 용인시 분동된 동백1동
  행정복지센터, 2025년 하반기 준공` 은 계획 발표인데 '준공 완료'가 됐다.
  유보 표현 목록이 '하반기 준공' 같은 꼴을 잡지 못한다. 시트에 준공예정일이
  있으면 새 강등 규칙이 막지만, 이 건은 예정일이 비어 있어 통과했다.
- **기사 본문이 없다.** 구글 뉴스 RSS의 description은 링크 태그뿐이라
  제목만으로 신호를 읽는다. 네이버는 본문을 줬다 — 예컨대 철거 판정은
  제목의 '철거공사 착수'와 본문의 '멸실 신고'를 같이 읽던 것이다.
- **근거가 구글 경유 링크다.** 피드에 기사 원문 주소가 없고 `<source url>`의
  매체 도메인까지가 전부다. 구글이 주소 형식을 바꾸면 그때까지 쌓인 근거
  링크가 같이 깨진다.
- **한 번에 5건 안팎만 온다.** 네이버는 10건이었다. `display`는 RSS가 받지
  않아 늘릴 방법이 없다.
- **`nara/naver.py`는 아무도 부르지 않는다.** `SearchResult`만 `google.py`가
  가져다 쓴다. 되돌릴 수 있게 남겨 둔 것이다 — `cli.py`의 import 한 줄을
  바꾸면 네이버로 돌아간다(키는 필요).
